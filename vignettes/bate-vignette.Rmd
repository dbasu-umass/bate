...
e <- 0.01
```
Note that we have not chosen a value for `Rlow`. This is because `Rlow` is chosen to be equal to `parameters$Rtilde` (the R-squared in the intermediate regression).
...
```
The value of $\delta^*$ printed above is just one value picked up from the function $\delta=f(R_{max}$. To see the graph of this function, we can use the function `delfplot()`.
```{r, out.width="100%"}
